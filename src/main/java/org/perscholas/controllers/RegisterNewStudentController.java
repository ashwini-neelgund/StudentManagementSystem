...
public class RegisterNewStudentController {
  ...
  /**
   * Post mapping for the new-student page. Checks for a valid student object, checks that it is not
   * a duplicate sign up and saves the student to the database.
   *
   * @param student Student to be persisted
   * @param result result to check for errors on
   * @param model the model
   * @return path to register.html if signup is successful, new-student if there are form errors, or
   *     home if the student is already signed up.
   */
  @PostMapping
